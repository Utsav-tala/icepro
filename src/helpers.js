...
// // ── Live balance: positive = agency owes us, negative = advance credit ────────
// export function computeBalance(agencyId, bills, payments) {
//   const billed = bills
//     .filter(b => b.agencyId === agencyId)
//     .reduce((s, b) => s + (b.total || 0), 0);   // b.total = actual bill amount only
//   const paid = payments
...
// // ── Balance display: −Rs.X (outstanding) or +Rs.X (advance) ──────────────────
...
//   const billAmt   = subtotal - discAmt;            // actual current bill amount
//   const prevBal   = Number(bill.prevBalance) || 0; // previous pending amount
//   const grandTotal = billAmt + prevBal;            // total amount due
...
//   // Previous balance row in totals section — only show if > 0
//   const prevBalRow = prevBal > 0
//     ? `<div class="trow" style="color:#c8181e;font-weight:700;"><span>Previous Pending Balance</span><span>+ ${prevBal.toFixed(2)}</span></div>`
//     : "";
...
//     .trow.grand span:last-child{font-family:'Playfair Display',serif;font-weight:800;font-size:16px;color:#c8181e;}
...
//         <div class="trow"><span>Current Bill Amount</span><span>${billAmt.toFixed(2)}</span></div>
//         ${prevBalRow}
//         <div class="trow grand"><span>Grand Total</span><span>${grandTotal.toFixed(2)}</span></div>
//       </div>
//     </div>
//     <div class="words-row"><b>Amount Due :</b>&nbsp;&nbsp;<i>${toWords(grandTotal)}</i></div>
...
//   const billAmt  = sub - disc;                     // actual current bill amount
//   const prevBal  = Number(bill.prevBalance) || 0;  // previous pending amount
//   const grandTotal = billAmt + prevBal;            // total amount due
...
export function printInvoice(bill, agency) {
  ...
  const discPct   = subtotal > 0 ? ((discAmt / subtotal) * 100).toFixed(2) : "0.00";
  const billAmt   = subtotal - discAmt;            // actual current bill amount
  const prevBal   = Number(bill.prevBalance) || 0; // previous pending outstanding
  const advUsed   = Number(bill.advanceUsed) || 0; // advance credit deducted
  const grandTotal = billAmt + prevBal - advUsed;  // total amount due
  const totalQty  = items.reduce((s, it) => s + Number(it.qty || 0), 0);
  const dateStr   = bill.createdAt?.toDate?.()?.toLocaleDateString("en-IN", { day:"2-digit", month:"2-digit", year:"numeric" })
                    || new Date().toLocaleDateString("en-IN", { day:"2-digit", month:"2-digit", year:"numeric" });
  ...
  // Previous balance / advance credit rows — only show if non-zero
  const prevBalRow = prevBal > 0
    ? `<div class="trow" style="color:#c8181e;font-weight:700;"><span>Previous Pending Balance</span><span>+ ${prevBal.toFixed(2)}</span></div>`
    : "";
  const advRow = advUsed > 0
    ? `<div class="trow" style="color:#065f46;font-weight:700;"><span>Advance Credit Applied</span><span>- ${advUsed.toFixed(2)}</span></div>`
    : "";
