...
.card{background:#fff;border:1px solid #f0dada;border-radius:16px;padding:20px;box-shadow:0 8px 30px rgba(200,24,30,0.03);}
.sc{background:#fff;border:1px solid #f0dada;border-radius:16px;padding:22px;transition:transform 0.25s cubic-bezier(0.2,1,0.3,1),box-shadow 0.25s cubic-bezier(0.2,1,0.3,1);box-shadow:0 8px 30px rgba(200,24,30,0.03);}
.sc:hover{transform:translateY(-4px);box-shadow:0 16px 40px rgba(200,24,30,0.08);}

.badge{padding:4px 10px;border-radius:20px;font-size:10px;font-weight:800;letter-spacing:0.5px;display:inline-block;text-transform:uppercase;}
...
.btn{padding:10px 22px;border-radius:12px;border:none;cursor:pointer;font-family:'Nunito',sans-serif;font-size:13px;font-weight:800;transition:all 0.25s cubic-bezier(0.2,1,0.3,1);}
.btn-red{background:linear-gradient(135deg,#c8181e,#9e1015);color:#fff;box-shadow:0 6px 16px rgba(200,24,30,0.25);}
.btn-red:hover{background:linear-gradient(135deg,#e03535,#c8181e);transform:translateY(-2px);box-shadow:0 10px 24px rgba(200,24,30,0.35);}
.btn-red:disabled{opacity:0.6;cursor:not-allowed;transform:none;box-shadow:none;}
.btn-yellow{background:linear-gradient(135deg,#f5c518,#d4a012);color:#9e1015;box-shadow:0 6px 16px rgba(245,197,24,0.25);}
...
.inp{background:#fff;border:1.5px solid #f0dada;border-radius:12px;padding:12px 16px;color:#1a0505;font-family:'Nunito',sans-serif;font-size:13px;outline:none;transition:border-color 0.2s,box-shadow 0.2s;width:100%;}
.inp:focus{border-color:#c8181e;box-shadow:0 0 0 4px rgba(200,24,30,0.08);}
.sel{background:#fff;border:1.5px solid #f0dada;border-radius:12px;padding:12px 16px;color:#1a0505;font-family:'Nunito',sans-serif;font-size:13px;outline:none;width:100%;cursor:pointer;transition:border-color 0.2s,box-shadow 0.2s;}
.sel:focus{border-color:#c8181e;box-shadow:0 0 0 4px rgba(200,24,30,0.08);}
...
.iopt{padding:12px 14px;cursor:pointer;border-bottom:1px solid #fdf0f0;transition:background 0.12s;}
.iopt:hover{background:#fff0f0;}
.iopt:last-child{border-bottom:none;}
.iopt-name{font-size:13px;font-weight:700;color:#1a0505;}
.iopt-rate{font-size:11px;color:#c8181e;font-weight:700;margin-top:2px;}

/* Desktop: controls wrapper flows children into parent grid */
.mobile-item-row-controls{display:contents;}
.brand-logo-wrap{display:block;}
.page-header-sticky{position:relative;z-index:10;background:#fdf5f5;}
...
@media(max-width:768px){
  ...
  .mbox{width:100%;max-width:100vw;border-radius:24px 24px 0 0;padding:24px 20px;max-height:96vh;animation:slideUp 0.35s cubic-bezier(0.2,1,0.3,1);}
  @keyframes slideUp{from{transform:translateY(100%);}to{transform:translateY(0);}}

  /* FIX 1: Mobile product dropdown — full-width fixed overlay */
  .iswrap{position:static!important;}
  .idrop{position:fixed!important;left:0!important;right:0!important;bottom:0!important;top:auto!important;max-height:50vh!important;border-radius:20px 20px 0 0!important;border:none!important;border-top:3px solid #c8181e!important;box-shadow:0 -12px 40px rgba(0,0,0,0.25)!important;z-index:1100!important;padding-bottom:env(safe-area-inset-bottom,12px)!important;}
  .iopt{padding:14px 18px!important;font-size:14px!important;}
  .iopt-name{font-size:14px!important;}
  .iopt-rate{font-size:12px!important;}
  
  /* Mobile items container redesign */
  .mobile-item-grid{display:block;}
  .mobile-item-header{display:none!important;}
  .mobile-item-row{display:grid;grid-template-columns:1fr auto;grid-template-rows:auto auto;gap:10px;padding:14px;border:1px solid #f0dada;border-radius:12px;margin-bottom:10px;background:#fff;}
  .mobile-item-row > div:nth-child(1){grid-column:1 / 2;grid-row:1;font-size:14px!important;} /* Product Name */
  .mobile-item-row > button{grid-column:2;grid-row:1;align-self:start;} /* Delete */
  .mobile-item-row-controls{display:flex!important;grid-column:1 / span 2;grid-row:2;gap:10px;align-items:center;background:#fef0f0;padding:8px 12px;border-radius:8px;}
  
  /* Mobile sticky page header */
  .page-header-sticky{position:sticky;top:0;z-index:50;padding-bottom:12px;margin-bottom:12px!important;border-bottom:1px solid #f0dada;}
  
  /* Mobile list layouts (Agencies/Billing grids) */
  .mobile-grid-hide{display:none!important;}
  .mobile-agency-row{display:grid!important;grid-template-columns:1fr auto!important;gap:10px!important;padding:16px!important;border:1px solid #f0dada!important;border-radius:14px!important;margin-bottom:12px!important;background:#fff!important;align-items:start!important;}
  .mobile-agency-row > div:nth-child(1){grid-column:1;grid-row:1;}
  .mobile-agency-row > div:nth-child(2){grid-column:1;grid-row:2;}
  .mobile-agency-row > div:nth-child(3){grid-column:2;grid-row:1;text-align:right;}
  .mobile-agency-row > div:nth-child(4){display:none;}
  .mobile-agency-row > div:nth-child(5){grid-column:2;grid-row:2;justify-self:end;}

  /* FIX 2: Bigger agency action buttons */
  .mobile-agency-actions{grid-column:1 / 3!important;grid-row:3!important;display:grid!important;grid-template-columns:1fr 1fr 1fr!important;gap:8px!important;margin-top:8px!important;padding-top:14px!important;border-top:1px dashed #f0dada!important;}
  .mobile-agency-actions .btn{font-size:12px!important;padding:10px 8px!important;width:100%!important;text-align:center!important;justify-content:center!important;}

  .mobile-bill-row{display:grid!important;grid-template-columns:1fr auto!important;gap:10px!important;padding:16px!important;border:1px solid #f0dada!important;border-radius:14px!important;margin-bottom:12px!important;background:#fff!important;align-items:start!important;}
  .mobile-bill-row > div:nth-child(1){grid-column:1;grid-row:1;}
  .mobile-bill-row > div:nth-child(2){grid-column:1;grid-row:2;}
  .mobile-bill-row > div:nth-child(3){grid-column:2;grid-row:1;text-align:right;}
  .mobile-bill-row > div:nth-child(4){grid-column:1;grid-row:3;}
  .mobile-bill-row > div:nth-child(5){display:none;}
  .mobile-bill-actions{grid-column:1 / 3;grid-row:4;display:flex;gap:8px;justify-content:flex-end;margin-top:4px;padding-top:12px;border-top:1px dashed #f0dada;}
  .mobile-bill-actions .btn{font-size:12px!important;padding:8px 16px!important;}

  /* FIX 3: Home page cards stacked vertically with scroll */
  .mobile-home-cards{grid-template-columns:1fr!important;}
  .mobile-home-cards .card{max-height:280px;overflow-y:auto;}

  /* FIX 5: Products page mobile card layout */
  .mobile-product-hide{display:none!important;}
  .mobile-product-row{display:grid!important;grid-template-columns:1fr auto!important;gap:8px!important;padding:16px!important;border:1px solid #f0dada!important;border-radius:14px!important;margin-bottom:12px!important;background:#fff!important;align-items:center!important;}
